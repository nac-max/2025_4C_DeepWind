...
def add_turbine(request):
    ...
    # 修复点：使用 get_or_create() 确保实例存在
    websiteStatistics, _ = WebsiteStatistics.objects.get_or_create(
        defaults={
            'statistics_time': 0,
            'statistics_visit': 0,
            'statistics_download': 0,
            'statistics_upload': 0,
            'statistics_train': 0,
            'statistics_forecast': 0,
            'statistics_uploadTurbine': 0,
        }
    )
...
def start_forecastTask(request):
    ...
    # 关键修改：确保 WebsiteStatistics 实例存在
    websiteStatistics, _ = WebsiteStatistics.objects.get_or_create(
        defaults={
            'statistics_time': 0,
            'statistics_visit': 0,
...
def get_realtimePredict(request):
    ...
    print(forecastModel.model_path)
...
def get_statistics(request):
    ...
    # 关键修改：确保实例存在
    webStatistics, _ = WebsiteStatistics.objects.get_or_create(
        defaults={
            'statistics_time': 0,
            'statistics_visit': 0,
            'statistics_download': 0,
    ...
    # 关键修改：直接操作序列化后的字典结构
    if len(websiteStatistics_dict) > 0:
        websiteStatistics_dict[0]['fields']['modelNum'] = modelNum
        websiteStatistics_dict[0]['fields']['turbineNum'] = turbineNum
        websiteStatistics_dict[0]['fields']['dataNum'] = dataNum
        websiteStatistics_dict[0]['fields']['taskNum'] = taskNum
        websiteStatistics_dict[0]['fields']['guestbookNum'] = guestbookNum

    return JsonResponse(websiteStatistics_dict, safe=False)
...
def get_useAge(request):
    ...
    try:
        ...
        # 使用正则匹配所有数字（处理多GPU情况）
        gpu_usages = re.findall(r'\d+', result.stdout)
        if gpu_usages:
            # 取第一个GPU的使用率（可根据需要修改为平均值或其他处理）
            gpu_usage = int(gpu_usages[0])

    except (subprocess.CalledProcessError, FileNotFoundError,
            subprocess.TimeoutExpired, Exception):
        # 忽略所有异常，保持gpu_usage为None
        pass
...
def hello_gpt(request):
    ...
    client = OpenAI(api_key="sk-0c80ccef2c1a45ddbccbca121c51d6fe", base_url="https://api.deepseek.com")
